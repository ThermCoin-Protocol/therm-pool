...
async function distributeTokensBatch(recipients, amount, batchSize) {
    ...
    for (let i = 0; i < recipients.length; i += batchSize) {
        ...
        console.log("Estimated gas: ", await tokenContract.methods.distributeReward(batch, web3.utils.toWei(amount.toString(), 'ether'), 0, batch.length).estimateGas({ from: account.address }) * 2);
        const tx = {
            ...
            gas: await tokenContract.methods.distributeReward(batch, web3.utils.toWei(amount.toString(), 'ether'), 0, batch.length).estimateGas({ from: account.address }) * 2,
        };
